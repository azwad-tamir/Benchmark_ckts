...
class vparser(object):
    ...
    def __init__(self, vcode_name):
        ...
        self.file = open(self.vcode_name)
        self.vcode = [line.strip() for line in self.file]
    ...
    def gethygraph(self):
        ...
        # reading each lines and populating list
        for i in range(len(self.vcode)):
            ...
            isnet = re.findall(net_pattern, self.vcode[i])

            if self.vcode[i].startswith("#"):
                continue

            elif isinput:
                inputs = self.vcode[i].replace('(','')
                inputs = inputs.replace(')','')
                content = re.findall(input_pattern, inputs)
                self.inputnets += content  # Get the inputs
                #print(content)
            elif isoutput:
                outputs = self.vcode[i].replace('(','')
                outputs = outputs.replace(')','')
                content = re.findall(output_pattern, outputs)
                self.outputnets += content  # Get the outputs
                #print(self.vcode[i])
            elif isnet:
                ...
                self.node_names.append(items[1])
                self.node_node.append('G'+str(self.gate_count))
                self.gate_count += 1
                self.getNets.append(items[2:items.__len__()] + items[0:1])
                self.wires.extend(items[2:items.__len__()] + items[0:1])

        self.wires = list(set(self.wires)) #removing repeated wires
        self.wires = list(set(self.wires) - set(self.inputnets))
        self.wires = list(set(self.wires) - set(self.outputnets))
        ...
        # adding input nodes:
        for gate in self.inputnets:
            self.G.add_node(gate, type='input', prob=0.5, prob_has=1)

        # adding output nodes:
        for gate in self.outputnets:
            self.G.add_node(gate, type='output', prob=0, prob_has=1)

        # adding gate nodes:
        i = 0
        for gate in self.node_node:
            self.G.add_node(gate, type=node_names[i], prob=0, prob_has=0)
            i += 1

        # adding wire nodes:
        for gate in self.wires:
            self.G.add_node(gate, type='wire', prob=0, prob_has=2)
        ...
        i = 0
        for net in self.getNets:
            self.G.add_edge(self.node_node[i], net[net.__len__() - 1])
            # print(node_node[i] + ' -->  ' + net[net.__len__()-1])
            for net_in in net[0:net.__len__() - 1]:
                self.G.add_edge(net_in, self.node_node[i])
        ...
        i = 0
        self.empty_wires = []
        for net in self.wires:
            if ((list(self.G.predecessors(net)).__len__() == 0) | (list(self.G.successors(net)).__len__() == 0)):
                # G.remove_node(net)
                self.empty_wires.append(net)
                i += 1
        print('Empty wires: ', i)

        # Removing wires from the graph:
        for net in self.wires:
            if self.G.has_node(net):
                if (list(self.G.predecessors(net)).__len__()) != 1:
                    print("Fetal Error: Check wire predecessor")

        for net in self.wires:
            if self.G.has_node(net):
                for net1 in list(self.G.successors(net)):
                    self.G.add_edge(list(self.G.predecessors(net))[0], net1)
                    print(list(self.G.predecessors(net))[0], ' --> ', net1)
                self.G.remove_node(net)
...
wires_main = obj.wires
empty_wires = obj.empty_wires
