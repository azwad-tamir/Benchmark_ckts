...
from scipy.spatial import distance as euc_distance
...
def parse_arguments():
    ...
    parser.add_argument(
        "--lef_dir",
        help="Directory to the lef file or text file with sizes",
        type=str,
        required=False,
    ...
    parser.add_argument(
        "--tile_n",
        help="number of tiles for the reinforcement agent, the circuit is partitioned into n tiles and then assigned"
             " to tiers, tile_n = tier_n * (k**2)",
        type=int,
        required=False,
        default=8
    )
    ...
    parser.add_argument(
        "--miv_reward",
        help="The length of a tier die over the length of an MIV",
        type=int,
        required=False,
        default=10
    )
...
if not jupyter:
    ...
    tile_n = args.get('tile_n')
    out_dir = args.get('out_dir')
    reinforcement = args.get('reinforcement')
    do_perm = args.get('do_perm')
    drop_threshold = args.get('drop_threshold')
    miv_reward = args.get('miv_reward')
    max_permutation = args.get('max_permutation')

if jupyter:
    # data_dir = 'files/Desktop/dhm_mini.v'
    data_dir = 'data/aes_cipher.v'
    # lef_dir = 'files/Desktop/celllist.txt'
    lef_dir = 'files/saed32_rvt.lef'
    tier_ns = 3
    tile_n = 27
    out_dir = 'results/'
    reinforcement = 1
...
for subject in subjects:
    for tier_n in [tier_ns]:
        ...
        try:
            ...
            print('LEF file not found, all gates are now considered to be of the same size.')
            dimensions = {}
        ...
        #                 break
        #         if faulty_flag == 0:
        #          /home/milad/PycharmProjects/deepmirna   ok_gates.append(gate)
        #
        #     # insert MIVs
        ...
        if reinforcement:
            ...
            for i in range(tile_n):
                threshold.append(threshold[-1]+1/tile_n)
            print(threshold)

            for i in range(len(data)):
                data[i]['tier'] = tile_n - 1
            ...
            for i in np.argsort(x_gate):
                tile_size += data[i]['size']
                data[i]['tier'] = target_tile
                if tile_size >= current_threshold * total_size:
                    target_tile += 1
                    if target_tile == tile_n - 1:
                        break
                    current_threshold = threshold[target_tile + 1]
            dummy_dict = {k:0 for k in range(tile_n)}
            for i in range(len(data)):
            ...
            # find number of connections between tiles
            connection_num_dict = {}
            all_nets = sorted(nets_wpin.keys())
            for i in range(tile_n):
                for j in range(tile_n):
                    connection_num_dict[(i, j)] = 0
            for net in all_nets:
                connected_gates_to_net = np.array(nets_wpin[net])
                connected_gates_tiers = tier_gate[connected_gates_to_net]
                gates_tier_dummy = []
                for tile_i in range(tile_n):
                    gates_tier_dummy.append(connected_gates_to_net[connected_gates_tiers == tile_i])
                    ...
                    if i in nonzero_lens:
                        for j in range(tile_n):
                            if j in nonzero_lens:
                                # connection_num_dict[(i,j)] += 1
                                connection_num_dict[(i,j)] += gates_lens[i] + gates_lens[j]
            print(connection_num_dict)

            if not do_perm:
                dummy_tiers = []
                for dummy_tier_i in range(tier_n):
                    dummy_tiers.extend([dummy_tier_i] * int(tile_n/tier_n))
                ...
                smallest_distance = 99999999999
                perm_counter = 0
                for p in perm:
                    tier_set = list(p)
                    dummy_distance = 0
                    for i in range(tile_n):
                        for j in range(i+1, tile_n):
                            if tier_set[j] != tier_set[i]:
                                dummy_distance += connection_num_dict[(i, j)]
                            else:
                                dummy_distance += miv_reward * connection_num_dict[(i, j)]
                    if dummy_distance < smallest_distance:
                        smallest_distance = dummy_distance
                        best_tier_arrangement = tier_set
                    if perm_counter % 10000 == 0:
                        print('Permutation ', perm_counter)
                    perm_counter += 1
                    if max_permutation != None:
                        if perm_counter >= max_permutation:
                            break
                print(best_tier_arrangement)

            if do_perm:
                # perm = [list(dummy) for dummy in list(permutations(np.arange(tile_n)))]
                perm = permutations(np.arange(tile_n))
                tile_per_tier = int(tile_n/tier_n)
                tile_per_row = int(tile_per_tier ** 0.5)
                tile_coords = []
                distance_per_tile = miv_reward / (tile_per_row-1)
                for tile_i in range(tile_n):
                    row_number = tile_i // tile_per_row % tile_per_row
                    column_number = tile_i % tile_per_row
                    height_number = tile_i // tile_per_tier
                    tile_coords.append([distance_per_tile *row_number, distance_per_tile * column_number, height_number])
                distance_k = {}
                for i in range(tile_n):
                    for j in range(i+1, tile_n):
                        distance_k[i, j] = euc_distance.euclidean(tile_coords[i], tile_coords[j])
                # distance_k_2 = {(0,1): 10, (0,2): 10, (0,3):15, (0,4): 1 , (0,5): 12, (0,6): 12, (0,7): 18, (1,2): 15,
                #               (1,3): 10, (1,4):12, (1,5): 1, (1,6):18, (1,7):12, (2,3):10, (2,4):12, (2,5):18,
                #               (2,6):1, (2,7):12, (3,4):18, (3,5):12, (3,6):12, (3,7):1, (4,5):10, (4,6):10,
                #               (4,7):15, (5,6):15, (5,7):10, (6,7):10}
                smallest_distance = 99999999999
                perm_counter = 0
                for p in perm:
                    dummy_distance = 0
                    ...
                    for i in range(tile_n):
                        for j in range(i+1, tile_n):
                            ...
                            dummy_distance += connection_num_dict[(i, j)] * distance_k[(tier_1, tier_2)]
                    if dummy_distance < smallest_distance:
                        smallest_distance = dummy_distance
                        best_tile_arrangement = tier
                    if perm_counter % 10000 == 0:
                        print('Permutation ', perm_counter)
                    perm_counter += 1
                    if max_permutation != None:
                        if perm_counter >= max_permutation:
                            break
                best_tier_arrangement = []
                for i in best_tile_arrangement:
                    best_tier_arrangement.append(i//tile_per_tier)
                # best_tier_arrangement = [0 if i < 4 else 1 for i in best_tile_arrangement]
                print(best_tier_arrangement)
