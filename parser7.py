...
# Creating expanded inputs and outputs. eg. [0:3] dcnt
input_net_expanded = []
input_pattern = r'\[(.+)\]'
input_pattern2 = r'\[(.+):(.+)\]'
input_pattern3 = r'\](.+)'
for input in allin:
    if re.findall(input_pattern, input):
        for i in range(0,int(re.findall(input_pattern2, input)[0][0])+1):
            input_net_expanded.append("{}[{}]".format(re.findall(input_pattern3, input)[0].replace(' ',''), i))
    else:
        input_net_expanded.append(input)

output_net_expanded = []
output_pattern = r'\[(.+)\]'
output_pattern2 = r'\[(.+):(.+)\]'
output_pattern3 = r'\](.+)'
for output in allout:
    if re.findall(output_pattern, output):
        for i in range(0,int(re.findall(output_pattern2, output)[0][0])+1):
            output_net_expanded.append("{}[{}]".format(re.findall(output_pattern3, output)[0].replace(' ',''), i))
    else:
        output_net_expanded.append(output)
...
##
## Creating expanded wires; similar to the input and output expansion
wires_net_expanded = []
wires_pattern = r'\[(.+)\]'
wires_pattern2 = r'\[(.+):(.+)\]'
wires_pattern3 = r'\](.+)'
for wire_net in wires_main:
    if re.findall(wires_pattern, wire_net):
        for i in range(0,int(re.findall(wires_pattern2, wire_net)[0][0])+1):
            wires_net_expanded.append("{}[{}]".format(re.findall(wires_pattern3, wire_net)[0].replace(' ',''), i))
            print(wires_net_expanded[-1])
    else:
        wires_net_expanded.append(wire_net)
...
# adding input nodes:
for gate in input_net_expanded:
    G.add_node(gate, type='input', prob=0.5, prob_has = 1)

#adding output nodes:
for gate in output_net_expanded:
...
# adding wire nodes:
for gate in wires_net_expanded:
...
# Analyzing empty wires:
i=0
empty_wires = []
for net in wires_net_expanded:
...
# Removing empty nodes: (nodes without any predecessors or successors)
for net in wires_net_expanded:
    if((list(G.predecessors(net)).__len__()==0)) and (list(G.successors(net)).__len__()==0):
        G.remove_node(net)
